####################################################################
#  Press  Space   to enable the game to detect your move
#  Check your terminal to see the game result
...
# constants to avoid any misspelling and to enforce consistency
class Move(Enum):
    SCISSOR = 'Scissor'
    ROCK = 'Rock'
    PAPER = 'Paper'
...
def main():
    ...
    with mp_hands.Hands(
        max_num_hands=1, # this is an additional parameter that specify how many hands we want to detect.
        model_complexity=0,
        ...
        while capture.isOpened():
            ...
            game_started = False
            # press Space bar to take the hand landmark and give the game result
            if cv2.waitKey(8) & 0xFF == 32: 
                game_started = True
            # press ESC key to terminate the program
            elif cv2.waitKey(3) & 0xFF == 27: # 27 == ESC
                break

            if not game_started:
                # show the frame without landmarks if the game is not started
...
# we use this function to map each hand_status type to an actual move
...
# this function generate a random move and check if the user won, lost, or
# neither
